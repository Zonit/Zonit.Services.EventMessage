...
/// <summary>
/// Implementacja kontekstu post�pu z time-based smooth progress.
/// Automatycznie wysy�a aktualizacje gdy % si� zmieni (max 100 razy).
/// </summary>
internal sealed class TaskProgressContext : ITaskProgressContext, IDisposable
{
        ...
        Guid taskId)
    {
        ...
        // Uruchom timer tylko je�li mamy kroki do �ledzenia
        if (_steps.Length > 0)
        {
            // Timer co 200ms sprawdza czy % si� zmieni�
    ...
    /// <summary>
    /// Wywo�ywane przez timer - aktualizuje post�p na podstawie up�ywu czasu.
    /// </summary>
    private void UpdateTimeBasedProgress()
    {
        ...
        // U�ywamy Monitor.TryEnter �eby unikn�� blokowania timera
        ...
        try
        {
            ...
            // Wysy�aj tylko gdy % si� zmieni (bez zmiany message)
    ...
    private int CalculateCurrentProgress()
    {
        ...
        // Oblicz post�p w ramach kroku (max 99% �eby nie przeskoczy� do nast�pnego)
    ...
    {
        // Wysy�aj tylko gdy % si� zmieni lub gdy jest nowa wiadomo��
    ...
    public void Dispose()
    {
        ...
        // Dispose timera poza lockiem �eby unikn�� deadlocka
