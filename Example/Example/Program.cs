...
internal class Program
{
    static async Task Main(string[] args)
    {
        ...
        var host = Host.CreateDefaultBuilder(args)
            ...
            {
                // ???????????????????????????????????????????????????????????????
                // Wszystkie trzy metody ZAWSZE dzia³aj¹ - nawet bez handlerów!
                // Source Generator automatycznie rejestruje handlery gdy istniej¹.
                // ???????????????????????????????????????????????????????????????
                
                services.AddCommandHandlers();  // Rejestruje Commands (CQRS)
                services.AddEventHandlers();    // Rejestruje Events (Pub/Sub)
                services.AddTaskHandlers();     // Rejestruje Tasks (Background Jobs)
            })
            .Build();
